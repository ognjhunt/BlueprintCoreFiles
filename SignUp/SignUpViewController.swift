...
//    }
    ...
    func saveUser(){
       ...
       // let trimmedFN = nameTextField.text!.trimmingCharacters(in: .whitespaces)
        ...
        if let uniqueIdentifier = userDefaults.string(forKey: "UniqueIdentifier") {
            ...
            blueprintCollection.whereField("creatorId", isEqualTo: "")
                .whereField("uniqueIdentifier", isEqualTo: uniqueIdentifier)
                .getDocuments { (snapshot, error) in
                    if let error = error {
                        print("Error retrieving blueprints: \(error.localizedDescription)")
                        return
                    }
                    ...
                    for document in documents {
                        let blueprintID = document.documentID

                        // Update the creatorId field with the user's ID
                        blueprintCollection.document(blueprintID).updateData(["creatorId": Auth.auth().currentUser?.uid ?? ""]) { error in
                            if let error = error {
                                print("Error updating creatorId for blueprint \(blueprintID): \(error.localizedDescription)")
                            } else {
                                print("Updated creatorId for blueprint \(blueprintID)")
                            }
                        }
                    }
                }
    ...
    func updateBlueprintCreatorId(){
        //place code in user sign up
