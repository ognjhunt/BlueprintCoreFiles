...
public class FirestoreManager {
    ...
    public static func getSessionAnchor(_ id: String, completion: @escaping (Anchor?) -> Void ) {
        
        let docRef = db.collection("sessionAnchors").document(id)
        
        docRef.getDocument { docSnapshot, err in
            if let err = err {
                print("ERROR - getSessionAnchor: \(err)")
            }
            
            guard let docSnapshot = docSnapshot, let userFirDoc = docSnapshot.data() else {
                print("ERROR - getSessionAnchor: Could not get sessionAnchor \(id)")
                return completion(nil)
            }
            
            completion(Anchor(userFirDoc))
        }
    }
    ...
    public static func getBlueprint(_ id: String, completion: @escaping (Blueprint?) -> Void ) {
        ...
        docRef.getDocument { docSnapshot, err in
            if let err = err {
                print("ERROR - getBlueprint: \(err)")
            }
            
            guard let docSnapshot = docSnapshot, let userFirDoc = docSnapshot.data() else {
                print("ERROR - getBlueprint: Could not get blueprint \(id)")
                return completion(nil)
            }
    ...
    private static var styleCache = NSCache<NSString, Style>()
    
    public static func getStyle(_ name: String, completion: @escaping (Style?) -> Void ) {
        styleCache.countLimit = 10
        styleCache.evictsObjectsWithDiscardedContent = true
        
        // Check if the model is in the cache
            if let cachedModel = styleCache.object(forKey: name as NSString) {
                // Return the cached model
                return completion(cachedModel)
            }

            let docRef = db.collection("styles").document(name)
            ...
            docRef.getDocument { docSnapshot, err in
                ...
                guard let docSnapshot = docSnapshot, let userFirDoc = docSnapshot.data() else {
                    print("ERROR - getModel: Could not get model \(name)")
                    return completion(nil)
                }

                let style = Style(userFirDoc)
                // Add the model to the cache
                styleCache.setObject(style, forKey: name as NSString)
                completion(style)
            }
    }
    
    private static var textureCache = NSCache<NSString, Texture>()
    
    public static func getTexture(_ id: String, completion: @escaping (Texture?) -> Void ) {
        textureCache.countLimit = 10
        textureCache.evictsObjectsWithDiscardedContent = true
        
        // Check if the model is in the cache
            if let cachedModel = textureCache.object(forKey: id as NSString) {
                // Return the cached model
                return completion(cachedModel)
            }

            let docRef = db.collection("textures").document(id)
            ...
            docRef.getDocument { docSnapshot, err in
                ...
                let model = Texture(userFirDoc)
                // Add the model to the cache
                textureCache.setObject(model, forKey: id as NSString)
                completion(model)
            }
    }
...
//            }
//
//            guard let data = data, let blueprint = UIImage(data: data) else {
...
//    }
    ...
    static func getBlueprintsInRange(centerCoord: CLLocationCoordinate2D, withRadius radiusInM: Double, completion: @escaping ([Blueprint]) -> Void) {
        ...
        let queries = queryBounds.map { bound -> Query in
            return self.db.collection("blueprints")
                .order(by: Blueprint.GEOHASH)
            ...
            query.getDocuments { querySnapshot, err in
                ...
                if let documents = querySnapshot?.documents {
                    ...
                    for document in documents {
                        ...
                        if let blueprint = try? Blueprint(document.data()) {
                    ...
                    print("ERROR - getBlueprintsInRange \(String(describing: err))")
                }
            }
        }
    }
...
//    }
    ...
    public static func updateNetwork(_ updateDoc: [String:Any], completion: @escaping (Bool) -> Void ) {
        ...
        db.document("blueprints/\(currentUserUid)").updateData(updateDoc) { err in
